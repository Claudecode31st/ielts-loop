  ...
  return (
    <div className="divide-y divide-[var(--border)]">
      {errors.map((error, idx) => {
        ...
        return (
          <div key={error.id} className="transition-colors">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                {/* Expand chevron */}
                <ChevronDown
                  className={`h-4 w-4 text-slate-400 shrink-0 mt-1 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
                />
              </div>
            </button>
            ...
            {/* Expanded panel */}
            {isOpen && (
              <div className="px-5 pb-5 bg-slate-50/60 border-t border-[var(--border)]">
                {/* Description */}
                <div className="pt-4 pb-3">
                  <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">What this means</p>
                    ...
                    <div className="space-y-2.5">
                      {examples.map((ex, i) => (
                        <div key={i} className="bg-white border border-[var(--border)] rounded-xl overflow-hidden">
                          <div className="flex items-start gap-2.5 px-3 py-2.5 border-b border-[var(--border)]">
                ...
                {examples.length === 0 && (
                  <p className="text-xs text-slate-400 italic mt-1">No specific examples recorded yet — submit more essays to see excerpts.</p>
                )}

                {/* CTA */}
                <Link
                  href="/exercises"
                  className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-700 transition-colors"
                >
